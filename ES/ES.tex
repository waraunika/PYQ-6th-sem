...
\section{Introduction}
  ...
  \subsection{Embedded Systems Overview}
    \begin{enumerate}[topsep=0pt, noitemsep]
      ...
      \item Draw the block diagram of a digital camera as an example of embedded system and explain it. \hfill [4] (\texttt{79 Ash})
        \lb Clarify the statement `Digital Camera is a good example of an Embedded System'. \hfill [3] (\bo{74 Bh})
        \lb How does digital camera satisfy embedded system characterstics. \hfill [1] (\bo{72 Ash})
...
\section{Single Purpose Processor Design Issues}
  ...
  \subsection{Custom Single-Purpose Processor Design}
    \begin{enumerate}[topsep=0pt]
      ...
      \item Design a single purpose processor that output Fibonacci numbers upto `n' places. Start design from function computing the desired result, FSDM and sketch a probable datapath.
        \enter\hfill [5] (\bo{77 Ch}) [8] (\bo{72 Ash}, 75 Ba)
  ...
  \subsection{Optimizing Custom Single-Purpose Processors}
    \begin{enumerate}[topsep=0pt, noitemsep]
      ...
      \item Explain in detail about the optimization of custom single purpose processor with a suitable example. \hfill [6] (79 Jth) [8] (\texttt{79 Ash})
      
      \item What are the parameter you consider for optimization of single purpose processor? \hfill [4] (\bo{75 Bh}) 

      \item What are the optimization opportunities in single purpose processor. \hfill [3] (\bo{77 Ch})
...
\section{General Purpose Processor Design Issues}
  ...
  \subsection{Programmer's View}
    \begin{enumerate}[topsep=0pt]
      ...
      \item Briefly explain programmer's consideration on general purpose processors.  \hfill [6] (\texttt{79 Ash})
    \end{enumerate}
  ...
  \subsection{Application-Specific Instruction-Set Processors}
    \begin{enumerate}[topsep=0pt]
      ...
      \item What ASIP serve as a compromise between the general-purpose processor and single purpose processors? \hfill [2] (\texttt{79 Ash})
...
\section{Memory}
  ...
  \subsection{Memory Write Ability and Storage Permanence}
    \begin{enumerate}
      ...
      \item Explain storage permanence and write ability of different memories. \hfill [3] (\texttt{79 Ash})
  ...
  \subsection{Memory Hierarchy and Cache}
    \begin{enumerate}
      \item Describe write policy and replacement algorithm used in cache memory. \hfill [5] (\texttt{79 Ash})
  ...
  \subsection{Microprocessor Intefacing: I/O Addressing, Interrupts, DMA}
    \begin{enumerate}
      \item What is the difference between memory-mapped I/O and standard I/O. \hfill [3] (\bo{72 Ash})
        \lb and isolated I/O. \hfill [3] (\texttt{79 Ash})
  ...
  \subsection{Arbitration}
    \begin{enumerate}
      ...
      \item Explain Priority arbitration. \hfill [3] (\bo{73 Bh}) [5] (\bo{80 Ch})
        \lb with block diagram and steps. \hfill [5] (\texttt{79 Ash})
...
\section{Real-Time Operating System (RTOS)}
  ...
  \subsection{Task, Process, and Threads}
    \begin{enumerate}
      \item Differentiate between process and thread. \hfill [3] (\bo{81 Ch, 79 Ch}, 70 Ma) [4] (73 Ma) [5] (\texttt{79 Ash})
  ...
  \subsection{Task Scheduling}
    \begin{enumerate}
      ...
      \item 3 processors with PIDs P1, P2, P3 with estimated completion time 10, 5, 7 ms respectively enter ready queue together. Now a new process with PID 4 with esitmated completion time 2 ms enters ready queue after 2 ms. Calculate waiting time and TAT. Also calculate average waiting time and average TAT using SJF preemptive scheduling algorithm. \hfill [6] (\texttt{81 Ash, 79 Ash})
      ...
      \item Three processes with process IDs P1, P2, P3 with estimated completion time 8, 6, 10 ms and priorities 0, 3, 2 (0-highest, 3-lowest) respectively, enters the ready queue together in order P1, P2, P3 (assume only P1 is present in the Ready queue when the scheduler picks it up and P2 and P3 enter ready queue after that). Now, a process P4 with estimated completion time 6 ms and priority 1 enters the ready queue after 5 ms of execution of P1. Calculate waiting time and TAT for each process and average waiting time and TAT. Assume there is no I/O waiting for the processes and priority-based scheduling. \hfill [5] (\bo{76 Bh})
        ...
        \lb estimated completion times: 10, 5, 7, 6. Priorities: 1, 3, 2, 0. P4 enters ready queue after 5 ms. 
          \enter\hfill [5] (\bo{73 Bh}) [8] (71 Ma)
  ...
  \subsection{Device Drivers}
    \begin{enumerate}
      \item What is device driver? \hfill [2] (\texttt{79 Ash})
    \end{enumerate}
...
\section{Control System}
  ...
  \subsection{Open-loop and Close-loop Control System Overview}
    \begin{enumerate}
      \item Differentiate between open loop and closed loop control system.
        \enter\hfill [2] (\bo{78 Ch}, 76 Ba) [3] (\bo{75 Bh, 72 Ash}) [4] (\bo{79 Ch}, 78 Po, 78 Po)
      ...
      \item How is closed loop CS better than open loop CS? Explain with example of cruise control system. \hfill [6] (\texttt{79 Ash})
  ...
  \subsection{Control System and PID Controllers}
    \begin{enumerate}
      \item What are the challenges of modeling a real physical system and how can you overcome it? 
        \enter\hfill [3] (\bo{74 Bh})

      \item Differentiate between P, PI, PD, and PID controllers. \hfill [4] (71 Ma)

      \item Explain different performance metrics of control system using suitable performance response diagram.
        \enter\hfill [4] (\bo{77 Ch})

      \item Explain controller design using PD and PI controller. \hfill [5] (\bo{71 Bh})

      \item Why is an integral controller needed in a control system? \hfill [2] (\texttt{79 Ash})
      ...
      \item Derive the condition of linear proportional controller for stability and avoid the oscillation.
        \enter\hfill [6] (\texttt{82 Ka})
    \end{enumerate}

  \subsection{Software coding of PID Controller}
    \begin{enumerate}
      \item Write an algorithm for a PID control. \hfill [4] (\bo{77 Ch})
      \lb Write an algorithm to implement the PID controller in software. \hfill [4] (\bo{73 Bh}) [5] (\bo{74 Ch})
  ...
  \subsection{PID Tuning}
    \begin{enumerate}
      ...
      \item What is the purpose of PID tuning, and what are the benefits of computer based control implementations? \hspace{14.7cm} [4] (70 Ma)
  ...
  \subsection{Design}
    \begin{enumerate}
      ...
      \lb Design an open-loop automobile cruise controller and derive the condition for no oscillation and reduction of road disturbance and determine performance parameter. \hfill [8] (79 Jth)

      \item Draw block diagram of a closed-loop control system to control speed in an automobile. \hfill [2] (\bo{80 Ch})
        \lb Draw the block diagram of closed-loop control system for speed control of an automobile and explain the conditions for no unbound and no oscillation showing all the design steps.
        \enter\hfill [8] (75 Ba, 73 Ma)
...
\section{IC Technology}
  ...
  \subsection{Semi-Custom ASIC IC Technology}
    \begin{enumerate}
      ...
      \item Explain the two broad categories of Semi-Custom IC technology. \hfill [3] (\bo{74 Ch}, 73 Ma)
  ...
  \subsection{Programming Logic Device (PLD) IC Technology}
    \begin{enumerate}
      ...
      \item What is photolithography? \hfill [1] (\texttt{81 Ash})
        \lb and what are its types? \hfill [3] (\texttt{79 Ash}) [4] (\texttt{80 Ash}, 72 Ma)
      ...
      \item Describe the alignment and exposure techniques in photolithographic process in IC fabrication.
        \enter\hfill [5] (71 Ma)

      \item Explain the steps of photo lithography process. \hfill [4] (\bo{78 Ch, 77 Ch}, 72 Ma) [5] (\bo{72 Ash}) [6] (\bo{76 Bh})
        \lb List the ten basic steps of photolithography. \hfill [3] (\texttt{81 Ash})
        \lb with appropriate figure. \hfill [4] (75 Ba)

      \item Explain Full-Custom, Semi-Custom and PLD schemes used in IC technology with steps and comparision. \hspace{14.6cm} [8] (\bo{73 Bh})

      \item List the three major IC technologies with brief definitions. \hfill [3] (70 Ma)

      \item Draw the logic circuit diagrams of NAND and NOR gates using CMOS and explain their operations. \hfill [5] (\texttt{79 Ash})

      \item Draw final structural view diagram of respective structure. \hfill [5] (79 Jth)
...
\section{Microcontrollers in Embedded Systems}
  ...
  \subsection{Intel 8051 microcontroller family, its architecture and instruction sets}
    \begin{enumerate}
      ...
      \item Explain the addressing modes used in 8051 microcontroller. \hfill [3] (\texttt{82 Ka}) [4] (\texttt{79 Ash})
  ...
  \subsection{Programming in Assembly Language}
    \begin{enumerate}
      ...
      \item Write an assembly program for 8051 to count number of 0's in an 8-bit data stored in ROM at 40H and result in RAM at 41H. \hfill [4] (\bo{79 Ch})
        \lb rom at 75H, ram at 45H. \hfill [4] (78 Po)
        \lb count 1's, rom at 75H, ram at 45H. \hfill [4] (\texttt{79 Ash})
  ...
  \subsection{A simple interfacing example with 7 segment display}
    \begin{enumerate}
      ...
      \lb Write an assembly program to display 0 to 9 in seven segments display. \hfill [5] (\bo{73 Bh}) [8] (\bo{71 Bh})
      ...
      \item Write 8051 program and draw circuit diagram to display number from 99 to 00 in seven segment display. The program should be written in both assembly and C. \hfill [8] (70 Ma)
        \lb Write an assembly program to design a down counter that counts from 99 to 00. \hfill [6] (\bo{77 Ch})
    \end{enumerate}
...
\section{VHDL}
  ...
  \subsection{VHDL overview}
    \begin{enumerate}
      ...
      \item What is structural style of architecture in VHDL? \hfill [3] (\bo{81 Ch}, \texttt{79 Ash})
  ...
  \subsection{Finite state machine design with VHDL}
    ...
    \item Write VHDL code for a D flip flop using process. \hfill [4] (79 Jth)
      ...
      \item Design a 3-bit up counter using T flip flop using structural modeling in VHDL. \hfill [5] (\texttt{79 Ash})
